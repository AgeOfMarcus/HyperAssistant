"""
Langchain Agent Manager for completing complex Tasks.
Coded by Marcus Weinberger, repurposed from:
    https://github.com/AgeOfMarcus/1337GPT
Read more about how it works:
    https://marcus.hashnode.dev/task-management-system-for-langchain

Modifications:
    * Added the TaskManager.check_task_needed function
        - uses TaskManager.CHECK_TASK_PROMPT
        - does not use TaskManager.load_json, looks for "true"
"""
...
class TaskManager(object):
    ...
    --------------
    """
    CHECK_TASK_PROMPT: str = '''
    Considering your final goal and current values, determine whether the following task is reasonable.
    Does the task need to be completed in order to satisfy your final goal? Can the task be completed with the information we have, in the current environment?
    Respond with a dictionary in valid JSON format with the following key: "continue" containing a bool, True if the task is reasonable, otherwise False.
    Do not include any other keys in your response.
    ...
    Be sure to include any important results with your Final Response, so that it can be used in the next task.
    ...
    def check_task_needed(self, task: str) -> bool:
        prompt = self._base() + self.CHECK_TASK_NEEDED_PROMPT.format(task=task)
        resp = self.llm(prompt)
